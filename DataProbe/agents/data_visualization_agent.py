...
from DataProbe.utils.plotly import plotly_from_dict
...
class DataVisualizationAgent(BaseAgent):
    ...
    def get_plotly_graph(self):
        ...
        """
        if self.response:
            plotly_graph = self.response.get("plotly_graph", None)
            if plotly_graph is None:
                return None
            
            # Check if it's a Figure object and convert to dict
            if hasattr(plotly_graph, 'to_dict'):
                return plotly_graph.to_dict()
            elif hasattr(plotly_graph, 'to_json'):
                import json
                return json.loads(plotly_graph.to_json())
            else:
                return plotly_graph
        return None
